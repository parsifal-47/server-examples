...
request(#s{sender = Pid} = State, _Meta) ->
  Pid ! {request, os:system_time(milli_seconds)},
  {nil, State}.
...
sender(Socket, Host, Port) ->
  receive
    {request, Start} -> Socket2 = send_n_get(Socket, Host, Port),
...
send_n_get(Socket, Host, Port) ->
  ...
  if Socket2 == undefined -> mzb_metrics:notify({"request.error", counter}, 1), undefined;
     true -> gen_tcp:send(Socket2, <<"I'm requesting the service">>),
...
request_sync(#s{socket = Socket} = State, _Meta) ->
  {Latency, ok} = timer:tc(?MODULE, send_n_get_sync, [Socket]),
  mzb_metrics:notify({"request.ok", counter}, 1),
  mzb_metrics:notify({"latency", histogram}, Latency div 1000),
  {nil, State}.

send_n_get_sync(Socket) ->
  gen_tcp:send(Socket, <<"I'm requesting the service">>),
    case gen_tcp:recv(Socket, 0, ?Timeout) of
        {ok, _Binary} -> ok;
        _ -> gen_tcp:close(Socket), nok
    end.